...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<rclcpp::Node>("sas_robot_driver_PX100");

    try
    {
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(), "::Loading parameters from parameter server.");

        sas::RobotDriverPX100Configuration configuration;

        sas::get_ros_parameter(node,"usb",configuration.usb);
        sas::get_ros_parameter(node,"protocol",configuration.protocol);

        std::vector<int64_t> ids_param;
        sas::get_ros_parameter(node, "ids", ids_param);

        configuration.ids.clear();
        configuration.ids.reserve(ids_param.size());

        for (int64_t v : ids_param) {
            configuration.ids.push_back(static_cast<uint8_t>(v));
        }

        sas::get_ros_parameter(node,"baud_rate",configuration.baud_rate);
        ...
        RCLCPP_INFO_STREAM_ONCE(node->get_logger(), "::Instantiating RobotDriverPX100.");
        auto robot_driver_PX100 = std::make_shared<sas::RobotDriverPX100>(configuration,
                                                                    &kill_this_process);
